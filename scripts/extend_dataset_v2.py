"""
Extended Dataset Generator - IR Bullet Hole Detection System

This script extends the existing realistic IR bullet hole dataset by generating
~10,000 additional samples using the same generation pipeline, ensuring:
...
def extend_dataset():
    """Extend existing realistic synthetic dataset with new samples"""
    
    # Verify dataset exists
    if not (DATASET_DIR / "annotations.csv").exists():
        print("ERROR: No existing dataset found at", DATASET_DIR)
        return False
    
    print("\n" + "=" * 80)
    print("EXTENDING IR BULLET HOLE DATASET - GENERATION & MERGE")
    print("=" * 80)
    ...
    try:
        ...
        num_bullet_holes = int(ADDITIONAL_SAMPLES * BULLET_HOLE_RATIO)
        num_non_bullets = int(ADDITIONAL_SAMPLES * NON_BULLET_RATIO)
        num_ambiguous = ADDITIONAL_SAMPLES - num_bullet_holes - num_non_bullets
        ...
        for idx in range(ADDITIONAL_SAMPLES):
            if (idx + 1) % 1000 == 0:
                print(f"  Progress: {idx + 1:5d} / {ADDITIONAL_SAMPLES}")
            
            # Use offset sample ID to avoid collisions
            sample_id = max_existing_id + 1 + idx
            label = labels[idx]
    ...
    try:
        ...
        diff_counts = merged_df['difficulty_level'].value_counts()
        print(f"\n? Difficulty distribution:")
        ...
        feature_cols = ['area', 'circularity', 'mean_intensity', 'contrast', 'entropy']
        for col in feature_cols:
            if col in new_df.columns:
                print(f"  {col:20s}: {new_df[col].min():8.2f} - {new_df[col].max():8.2f} (mean={new_df[col].mean():8.2f})")
    ...
    print(f"\n? STATISTICS:")
    print(f"  Total bullet holes:  {(merged_df['label'] == 'bullet_hole').sum():,} ({100*(merged_df['label'] == 'bullet_hole').sum()/len(merged_df):.1f}%)")
    print(f"  Total non-bullets:   {(merged_df['label'] == 'non_bullet').sum():,} ({100*(merged_df['label'] == 'non_bullet').sum()/len(merged_df):.1f}%)")
    print(f"  Total ambiguous:     {(merged_df['label'] == 'ambiguous').sum():,} ({100*(merged_df['label'] == 'ambiguous').sum()/len(merged_df):.1f}%)")
