"""
Realistic IR Bullet Hole Dataset Generator - QUICK TEST VERSION

Generates a small test dataset (100 samples) for quick validation before 
generating the full 10,000 sample dataset.
"""
...
DATASET_SIZE = 100  # Small test set
IMAGE_SIZE = 256
DATASET_DIR = Path("dataset_realistic_test")
...
def generate_bullet_hole_physics(center, radius, background_intensity, 
    ...
    """Generate physically-realistic bullet hole."""
...
def compute_features(binary_mask, intensity_image):
    """Compute features from mask and intensity image."""
    features = {}
...
def generate_synthetic_sample(sample_id, label_type):
    ...
    if label_type == 'bullet_hole':
        ...
        # Simple non-bullet: just a small scratch
...
def generate_test_dataset():
    ...
    print("=" * 70)
    print("REALISTIC IR BULLET HOLE DATASET - TEST VERSION")
    print("=" * 70)
    print(f"\nGenerating {DATASET_SIZE} test samples...")
    ...
    for sample_id in range(DATASET_SIZE):
        if (sample_id + 1) % 10 == 0:
            print(f"  Progress: {sample_id + 1:3d} / {DATASET_SIZE}")
    ...
    print(f"  Background (mean): {df['mean_intensity'].quantile(0.5):.1f}")
    print(f"  Hole center (min): {df['min_intensity'].mean():.1f} ± {df['min_intensity'].std():.1f}")
    print(f"  Contrast: {df['contrast'].mean():.1f} ± {df['contrast'].std():.1f}")
    
    print(f"\n? TEST DATASET READY FOR QUALITY VERIFICATION")
    print(f"  Check: dataset_realistic_test/images/")
